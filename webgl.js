...
// Agora o Three.js também é carregado no mobile.
// compact3D continua sendo usado apenas para reduzir a complexidade.
if (!reduceMotion) {
...
if (
  ...
  function updateBounds() {
    ...
    const top = element =>
      element.getBoundingClientRect().top + pageY;

    bounds = {
      wifiStart:
        top(sections.services) -
        innerHeight * .72,

      wifiEnd:
        top(sections.process) +
        sections.process.offsetHeight * .1,
  ...
  function updateFallback(
    ...
    const position = (
      ...
      element.style.transform =
        `translate(${x}vw, ${y - 50}vh)
         perspective(900px)
         rotateX(${progress * 390}deg)
         rotateY(${progress * 78}deg)
         scale(${mobile ? .31 : .25})`;
    };
  ...
  try {
    // Mobile agora usa o 3D real.
    // O fallback só acontece para aparelhos realmente muito limitados.
    if (veryLowPower3D) {
      throw new Error(
        'Fallback 2D ativado por modo de economia ou dispositivo de baixa potência.'
      );
    }
    ...
    console.info(
      'WebGL indisponível; fallback 2D ativado.',
      error
    );
  }
